...
Shader Shader::loadVertexShader(const std::string& fileName)
{
	return loadShader(GL_VERTEX_SHADER, fileName);
}

Shader Shader::loadFragmentShader(const std::string& fileName)
{
...
Shader Shader::loadShader(std::uint32_t type, const std::string& fileName)
{
	std::ifstream file(fileName);
	if (!file.is_open())
		return { "Unable to open shader file '" + fileName + "'!" };
